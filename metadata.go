...
// This is the identifier charset plus the forward slash, which metadata keys
// use as a namespace separator (e.g. "formance.com/reviewed"). The slash is
// not allowed in ledger names because those are rendered as URL path segments
// where it would be a separator.
...
// charset plus the forward slash, [a-zA-Z0-9._:/-]. The strict charset
// excludes whitespace, free punctuation, and UTF-8 multibyte sequences; this
// also rules out null bytes that would corrupt null-terminated key encodings
// in storage backends, and keeps keys safe to render in CLI output, logs, URLs
// and HTTP/2 metadata trailers.
